...
def scrape_jamuna(visited_urls, config):
    feed_url = 'https://news.google.com/rss/search?q=site:jamuna.tv&hl=bn&gl=BD&ceid=BD:bn'
    ...
    for entry in feed.entries[:5]:
        ...
        try:
            ...
            if len(text) > 100:
                news_items.append({
                    'title': entry.title.replace(' - Jamuna Television', '').strip(),
                    'text': text,
                    'url': real_url,
                    'image': article.top_image if article.top_image else None
                })
        except Exception as e:  # noqa: BLE001
            print(f"Error scraping Jamuna TV ({real_url}): {e}")
